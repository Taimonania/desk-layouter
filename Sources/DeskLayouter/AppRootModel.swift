...
/// The observable app-level hub for the window's root: which full-window screen is
/// shown (board vs. Settings) and the app-level preferences the Settings screen
/// edits. It wraps the pure `AppNavigation` seam and the `AppStateStore` so the
...
final class AppRootModel: ObservableObject {
    /// The current full-window screen. Mutating the value republishes because it is
    /// `@Published`, so the root view swaps content in response.
    ...
    /// Swaps the window to the full-window Settings screen.
    func showSettings() {
        navigation.showSettings()
    }

    /// Returns to the board (the Settings screen's "Done" control).
    func showBoard() {
